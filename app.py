#!/usr/bin/env python
# A tool for gathering information about hardware present on the node where the
# inspection was scheduled.
# TODO: rewrite this to a statically linked binary so Python interpreter is not needed.
# We cannot rely on anything that is not in the standard library.
# Based on cpuinfo from numexpr, additionally adjusted.
#
###################################################################
...
def key_value_from_command(cmd, sep, successful_status=(0,), stacklevel=1):
    ...
    for line in command_by_line(cmd, successful_status=successful_status, stacklevel=stacklevel + 1):
        line_set = [s.strip() for s in line.split(sep, 1)]
        if len(line_set) == 2:
            d[line_set[0]] = line_set[1]
    return d
...
class LinuxCPUInfo(CPUInfoBase):
    ...
    def _has_Altivec(self):
        return re.match(r".*altivec\ supported.*", self.info[0]["cpu"]) is not None
...
class SunOSCPUInfo(CPUInfoBase):
    ...
    def __init__(self):
        if self.info is not None:
            return
        info = command_info(
            arch="arch", mach="mach", uname_i=["uname", "-i"], isainfo_b=["isainfo", "-b"], isainfo_n=["isainfo", "-n"]
        )
